...
#making the count matrix with the forward reads from the skin
#setwd('/Users/anthony/Desktop/RNAseq/BenSandra/Data&Figures_03242021/SKIN/reads files/forwardReads/')
setwd('/Users/anthony/Desktop/RNAseq/BenSandra RNAseq/Code&Data&Figures/SKIN/reads files/forwardReads/')
#getting the list of files from the forward reads 
list_of_count_files <- list.files()

#load each of the skin files as their own data frame 
#row names are gene/ensembl names 
#I will only use the column 2 count 

for (i in list_of_count_files) {
  
  assign(i, read.table(i,skip = 4, header = FALSE, sep = "", row.names = 1))
  
}

#make a list of gene names to use as row names later in the big count matrix
geneNames <-  row.names(skin01.txt)


#merge V2 columns from selected data frames into one single data frame 
#I chose the V2 column becuase on the following website https://www.biostars.org/p/218995/
#the first count column, V2, is for a non-stranded experiment, and according to the protocol we used, it is not stranded. 
#https://www.nugen.com/sites/default/files/M01442v2_User_Guide:_Universal_Plus_mRNA-Seq_4274.pdf
#https://www.nugen.com/products/technology
#since we don't have strand specific protocol - we dont care about strand
#columns will be renamed to treatments & replicate numbers 


#only using forward reads (64 of 128 files)
df <- bind_cols(skin01.txt$V2,
                skin02.txt$V2,
                skin03.txt$V2,
                skin04.txt$V2,
                skin05.txt$V2,
...
#let's save this data frame so we can start with it and don't have to redo this step 
...
zz <- join(a,df,by ='ensembl_gene_id' )
write.xlsx(zz,'SkinCountMatrix.xlsx')
#zz is the new count matrix
#saving the count matrix to the general directory 

setwd('/Users/anthony/Desktop/RNAseq/BenSandra/Data&Figures_03242021/SKIN/reads files/')
...
#making the column names the names of the files 
colnames(zz) <- list_of_count_files
...
#reading in the metaData for the count matrix
experiment_description <- read.csv("experiment_description.csv") #loading in column/sample metadata

#making metadata / colData for deseq objects with more appropriate variable names 

colData <- experiment_description
day4colData <- experiment_description[1:16,]
day7colData <- experiment_description[17:32,]
day11colData <- experiment_description[33:48,]
day23colData <- experiment_description[49:64,]
...
plotPCA(vsd4,intgroup = c("Treatment"),ntop = nrow(vsd)) +
  geom_text(aes(label=name),vjust=2)
plotPCA(vsd7,intgroup = c("Treatment"),ntop = nrow(vsd)) +
  geom_text(aes(label=name),vjust=2)
plotPCA(vsd11,intgroup = c("Treatment"),ntop = nrow(vsd)) + 
  geom_text(aes(label=name),vjust=2)
plotPCA(vsd23,intgroup = c("Treatment"),ntop = nrow(vsd))+ 
  geom_text(aes(label=name),vjust=2)
